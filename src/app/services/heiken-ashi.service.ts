...
export class HeikenAshiService {
  ...
  calculateHeikenAshi(stockName): Promise<any> {
    const returnPromise = new Promise((resolve, reject) => {
      ...
      this.apiService.getStockData(stockName).subscribe(
        (next: any) => {
          ...
          timeStampArr.forEach((timeStamp, timeIndex) => {
            ...
            if (quoteArray.length >= 1) {
              ...
              stockDataHA.open =
                Math.round(listData.open[timeIndex] * 100) / 100;
              stockDataHA.high =
                Math.round(listData.high[timeIndex] * 100) / 100;
